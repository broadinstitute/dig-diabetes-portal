...
var variantProcessing = (function () {
                ...
                if (splitKey.length > 3) {
                    ...
                    if (splitKey[1] !== 'NONE') {
                        var phenotypeMap = {'property': splitKey[0],
                            ...
                            'translatedName': splitKey[2],
                            'meaning': splitKey[3],
                            'samplegroup': splitKey[4],
                            'pValue': data [i].count};
    ...
    fillTraitsPerVariantTable = function (vRecO, show_gene, show_exseq, show_exchp, traitRootUrl) {
        ...
        console.log("fillTraitsPerVariantTable", vRecO);

        var vRec = deconvoluteVariantInfo(vRecO);
        console.log("vRec", vRec)
        var structureForBuildingTable = buildIntoRows(vRec);
        console.log("structureForBuildingTable", structureForBuildingTable);
        var rowCounter = 0;
        for (var phenotypeName in structureForBuildingTable["phenotypeRows"]) {
            if ((structureForBuildingTable["phenotypeRows"].hasOwnProperty(phenotypeName)) &&
                ...
                for (var i = 0; i < rowsPerPhenotype.length; i++) {
                    ...
                    var convertedTrait = phenotypeName;
    ...
    var addTraitsPerVariantTable = function (vRecO, traitsPerVariantTable, traitRootUrl, existingRowCount) {
        var retVal = [];
        if (!vRecO) {   // error condition
            return;
        }
        ...
        for (var phenotypeName in structureForBuildingTable["phenotypeRows"]) {
            if ((structureForBuildingTable["phenotypeRows"].hasOwnProperty(phenotypeName)) &&
                ...
                for (var i = 0; i < rowsPerPhenotype.length; i++) {
                    ...
                    // field 1: data set
                    var datasetHolder = document.createElement('div');
                    var classString;
                    if (firstOfMultiPhenotypes) {
                        classString = 'vandaRowHdr sgIdentifierInTraitTable indexRow'
                        ...
                        classString = 'vandaRowHdr sgIdentifierInTraitTable'
                    }
                    $(datasetHolder).attr({
                        id: "traitsTable" + rowCounter,
                        class: classString,
                        datasetname: row.samplegroup,
                        phenotypename: phenotypeName,
                        samplegroup: row.samplegroup,
                        rowsPerPhenotype: rowsPerPhenotype.length,
                        convertedSampleGroup: convertedSampleGroup
                    });
                    // need to do this because the DataTable api is expecting HTML strings, not
                    // HTML elements >.>
                    retVal.push(datasetHolder.outerHTML);
