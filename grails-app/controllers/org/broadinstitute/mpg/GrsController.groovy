...
class GrsController {
    ...
    def getGRSListOfVariantsAjax() {
        ...
        List <String> variantList = restServerService.getGrsVariants(grsName)
        ...
        for (String varId in variantList){
            ...
            recordPerVariants << "{\"d\":1,\"dataset\":1,\"pVals\":[{\"count\":\"${idPieces[0]}\",\"level\":\"CHROM\"},"+
                                    ...
                                    "{\"count\":\"${idPieces[3]}\",\"level\":\"Effect_Allele\"}]}"
        }

        JsonSlurper slurper = new JsonSlurper()
        String codedVariantList = """{"variantInfo":{
                                        "results":["${recordPerVariants.join("\",\"")}"]}}"""
